...
class DressCodeController extends Controller
{
     ...
     */
    public function storeDressCode(Request $request, Events $events): DressCodeResource|JsonResponse
    {
        try {
            $data = $request->validate([
                'dressCodeType' => 'required|string|max:255',
                'description' => 'nullable|string',
                'reservedColors' => 'nullable|array',
                'dressCodeImages' => 'nullable|array',
            ]);
            
            $dressCode = $this->dressCodeServices->createDressCode($events, $data) ?? null;
     ...
     * @param Events $events The event associated with the dress code being updated.
     ...
     */
    public function updateDressCode(Request $request, Events $events, DressCode $dressCode): DressCodeResource|JsonResponse
    {
        try {
            $data = $request->validate([
                'dressCodeType' => 'required|string|max:255',
     ...
     */
    public function destroyDressCode(Events $events, DressCode $dressCode): DressCodeResource|JsonResponse
    {
        try {
